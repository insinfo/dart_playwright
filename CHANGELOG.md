...
- **Tracing that the official viewer opens**: `context.tracing.start/startChunk/stopChunk/stop` writes upstream's trace archive — `trace.trace`, `trace.network` and `resources/<sha1>.<ext>` — so `npx playwright show-trace` reads it. Actions of `Page`, `Locator` and `BrowserContext` as `before`/`after` pairs with their errors, network requests as HAR entries with the response bodies attached, console messages, page errors, dialogs, downloads and page open/close. With `snapshots: true`, the DOM of every frame before and after each action, with the acted element marked and `<iframe>`s pointing at their child snapshots; with `sources: true`, the Dart stack of each action and the `.dart` files it points at, which is what fills the viewer's Source tab. Ports `tracing.ts`, `snapshotter.ts`, `snapshotterInjected.ts`, `harTracer.ts`, `serializedFS.ts` and `instrumentation.ts`; the format is `traceV9.ts`, the newest a released viewer reads. Not ported: the screencast filmstrip (`screenshots: true` here is upstream's `snapshots: { screen: true }`, one PNG per action phase) and video recording.
