...
- **Domínios CDP**: Implementação das conexões `CrConnection`, sessões `CDPSession`, manipulação de contextos, páginas, frames e gerência de rede (`CrNetworkManager`).
- **API Pública**: Primeira versão orientada a objetos exposta para os usuários (`Playwright`, `BrowserType`, `Browser`, `BrowserContext`, `Page`, `Locator`, `Frame`, `Request`, `Response`).
