...
class WkSession extends EventEmitter {
  ...
  WkSession(this.connection, this.sessionId, [this.targetType = 'page']) {
...
class WkConnection extends EventEmitter {
  ...
  int _lastId = 0;
  bool _isClosed = false;
  late final WkSession rootSession;

  WkConnection(this.transport) {
    rootSession = WkSession(this, '', 'browser');
    transport.onMessage.listen(_onMessage);
    transport.onClose.listen(_onClose);
  }

  WkSession createSession(String sessionId, [String targetType = 'page']) {
    final session = WkSession(this, sessionId, targetType);
