...
/// The DOM snapshot streamer, as it runs inside the page.
///
/// This is upstream's `frameSnapshotStreamer` with the TypeScript annotations
/// removed and the class fields moved into the constructor; the algorithm is
/// unchanged, because the shape it produces is the contract the viewer reads
/// (`NodeSnapshot` in `traceV9.ts`): a node is `[name, attrs, ...children]`, a
/// text node is a string, and an unchanged subtree is `[[snapshotsAgo,
/// nodeIndex]]` — a back-reference that is what keeps a trace of a hundred
/// actions from holding a hundred copies of the same page.
///
/// The `__playwright_*` attributes are not decoration either: the viewer reads
/// them back to restore what the HTML does not carry — the live `value` of an
/// input, a checkbox's `checked`, scroll offsets, an open shadow root, the
/// element an action targeted.
///
/// One thing this port does *not* do is install the streamer before the page's
/// own scripts: `addInitScript` is not ported yet, so the driver installs it on
/// first capture in each document. See `snapshotter.dart` for what that costs.
const String kSnapshotStreamerSource = r'''
function frameSnapshotStreamer(snapshotStreamer, removeNoScript) {
  ...
  class Streamer {
    constructor() {
      ...
      this._modifiedStyleSheets = new Set();
      // To avoid invalidating due to our own reads.
      this._readingStyleSheet = false;
      this._observedDocument = undefined;
    ...
    captureSnapshot(reset) {
      ...
      const visitNode = (node) => {
        ...
        if (nodeType === Node.ELEMENT_NODE) {
          const element = node;
          if (element.localName.includes('-') && window.customElements && window.customElements.get(element.localName))
          ...
          if (element.__playwright_target__ === this._targetGeneration) {
        ...
        if (nodeName === 'IFRAME' || nodeName === 'FRAME') {
          const frameId = node[kSnapshotFrameId];
          const name = 'src';
        ...
        if (nodeType === Node.ELEMENT_NODE) {
          ...
          for (let i = 0; i < element.attributes.length; i++) {
            ...
            if (nodeName === 'META')
              value = this.__sanitizeMetaAttribute(name, value, element.httpEquiv || '');
      ...
      const visitStyleSheet = (sheet) => {
        ...
        data.ref = [snapshotNumber, nodeCounter++];
        const attrs = {};
        attrs[kStyleSheetAttribute] = cssText;
        return { equals: false, n: ['template', attrs] };
      };
